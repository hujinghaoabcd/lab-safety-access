  ...
  for (let i = 0; i < particles.length; i++) {
    ...
    for (let j = i + 1; j < particles.length; j++) {
      ...
      if (distance < 150) {
        ...
        ctx.strokeStyle = `rgba(160, 174, 192, ${0.4 * (1 - distance / 150)})`
        ctx.lineWidth = 1
        ctx.stroke()
      }
    }
...
<template>
  <div class="login-page">
    ...
    <div class="login-card">
      <!-- 左侧插图区域（用 @/assets/logo2.svg 替换原 data-center 插画） -->
      ...
      <div class="login-form-section">
        <div class="login-content">
          <h1 class="login-title">考试后台管理系统</h1>
...
.illustration-img {
  width: 77%;
  height: 77%;
  object-fit: contain; // 若需铺满可改为 cover
  display: block;
}
